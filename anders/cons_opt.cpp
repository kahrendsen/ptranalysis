...
void Anders::cons_opt(){
  ...
  hvn(0);
  //Run HRU until it can no longer remove 1000 constraints.
  hr(1, 1000);
...
hash_map<bitmap, u32> lbl2pe;
//The RHS of any GEP constraint is mapped to a label.
hash_map<pair<u32, u32>, u32> gep2pe;
...
void Anders::factor_ls(){
  ...
  //Map each (src,off) pair (for loads) or (dest,off) (for stores)
  //  to the temp node used to factor it.
  hash_map<pair<u32, u32>, u32> factor_temp;
  hash_map<pair<u32, u32>, set<u32> >::const_iterator it, ie;
  for(it= loads.begin(), ie= loads.end(); it != ie; ++it){
    u32 t= factor_ls(it->second, it->first.first, it->first.second, 1);
    //If !t, it wasn't factored.
    if(t) factor_temp[it->first]= t;
  }
  for(it= stores.begin(), ie= stores.end(); it != ie; ++it){
    u32 t= factor_ls(it->second, it->first.first, it->first.second, 0);
    if(t) factor_temp[it->first]= t;
  }
  ...
  FORN(i, old_icall_cons.size()){
    Constraint &C= old_icall_cons[i].first;
    //Was this cons. factored?
    //Look for its (src,off) pair (or (dest,off) for stores).
    hash_map<pair<u32, u32>, u32>::const_iterator i_ft= factor_temp.
        find(make_pair(C.type == load_cons ? C.src : C.dest, C.off));
    if(i_ft != factor_temp.end()){
      if(C.type == load_cons)
        C.dest= i_ft->second;
      else
        C.src= i_ft->second;
...
//Returns the temp node created, or 0 if not factored.
u32 Anders::factor_ls(const set<u32> &other, u32 ref, u32 off, bool load){
  u32 szo= other.size();
  assert(szo);
  //dest (for load) or src (for store) will be filled in below.
  Constraint C(load ? load_cons : store_cons, ref, ref, off);
  if(szo < factor_ls_min_sz){
