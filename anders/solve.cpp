...
void Anders::solve_init(){
  ...
  if(WL_ORDER == WLO_PRIO)
    assert(WL_PRIO == WLP_LRF && "only LRF priority supported");
...
void Anders::solve(){
  ...
  while(1){
    ...
    if(USE_MEM_TIME && (n_node_runs&1023) == 0){
      if((get_cpu_time() - start_time)/1000000 > SOLVE_TIME_LIMIT){
        fprintf(stderr, "!! Running longer than %us, aborting.\n",
            SOLVE_TIME_LIMIT);
        break;
      }
    }
    ...
    if(USE_MEM_TIME && (n_node_runs%10000) == 0){
      if(get_mem_usage() > SOLVE_RAM_LIMIT){
        fprintf(stderr, "!! Used over %uM of RAM, aborting.\n",
            SOLVE_RAM_LIMIT);
        break;
      }
      putc('.', stderr);
    ...
    if(lcd_starts.size() >= lcd_sz || n_node_runs - last_lcd >= lcd_period)
      run_lcd();
  }
  if(USE_MEM_TIME)
    PUTS("]\n");
...
bool Anders::solve_ls_cons(u32 n, u32 hcd_rep, bdd d_points_to,
  ...
  //Solve the load/store without offset.
  assert(!I);
  bool chg= 0;
  for(const u32 *ip= pdpts; ip != edpts; ++ip){
    u32 rn= get_node_rep(*ip);
    if(load){
      chg |= add_copy_edge(rn, dest);
    }else{
...
//------------------------------------------------------------------------------
//Solve a load/store cons. with offset. This is like the main loop in
//  solve_ls_cons() but more complex; it was separated for profiling.
NOINLINE void Anders::solve_ls_off(bdd d_points_to, bool load,
